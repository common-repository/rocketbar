...
= 170425 =
* Plugin now shows posts of ALL Post Types
* Debug / Local-site mode through `define('ROCKETBAR_ALLOW_ALL')`.
  This allows developers to run the site in a completely insecure way to allow quick navigations and logins for any visitor. Use with caution.
* Search command added through `s`. Search your site quickly by typing in `s <searchcommand>`
* REMOVED Cron Job for updating the Rocketbar cache. This can slow sites down drastically.
  Instead, we update Rocketbar's cache on every page load possible. (to be updated)
